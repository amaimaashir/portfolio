...
//  SANITY CONFIG
// ================================
const SANITY_PROJECT_ID = "zzi4rw4f";
...
// ================================
function buildGithubCard(project, index) {
...
// ================================
function buildAiCard(project, index) {
  const imageUrl  = buildImageUrl(project.thumbnail?.asset?._ref);
  const gradients = ["gradient-3", "gradient-4", "gradient-1", "gradient-2"];
  const gradient  = gradients[index % gradients.length];
...
async function loadGithubProjects() {
  ...
  const query = `*[_type == "githubProject"] | order(order asc, _createdAt desc) {
    title,
    description,
    githubUrl,
    liveUrl,
  ...
  container.innerHTML = projects.map((p, i) => buildGithubCard(p, i)).join("");
...
async function loadAiWebsites() {
  ...
  const query = `*[_type == "aiWebsite"] | order(order asc, _createdAt desc) {
    title,
    description,
    liveUrl,
    toolsUsed,
  ...
  container.innerHTML = projects.map((p, i) => buildAiCard(p, i)).join("");
